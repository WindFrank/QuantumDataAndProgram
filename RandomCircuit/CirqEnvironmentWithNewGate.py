...
'''
get_qasm_run_result: Get the run result from qiskit qasm_simulator
It's return is a real number probability vector.
'''
...
def read_circuit_string_to_cirq(circuit_string, width):
    ...
    while i < len(circuit_string):
        if circuit_string[i] == 'C' and circuit_string[i + 1] == 'Z':
            ...
            mapping = {
                'X': cirq.X(qubits[qubit]),
                'Y': cirq.Y(qubits[qubit]),
                'Z': cirq.Z(qubits[qubit]),
                'H': cirq.H(qubits[qubit]),
                'S': cirq.S(qubits[qubit]),
                'SD': cirq.rz(-1 * np.pi / 2).on(qubits[qubit]),
                'T': cirq.T(qubits[qubit]),
                'TD': cirq.rz(-1 * np.pi / 4).on(qubits[qubit]),
                'X2P': cirq.rx(np.pi / 2).on(qubits[qubit]),
                'X2M': cirq.rx(-1 * np.pi / 2).on(qubits[qubit]),
                'Y2M': cirq.ry(-1 * np.pi / 2).on(qubits[qubit]),
                'Y2P': cirq.ry(np.pi / 2).on(qubits[qubit])
            }
            operate = mapping[gate]
            circuit.append(operate)
            i = i_later
    return circuit
...
def get_bas(circuit_code, width, environment, repetitions):
...
def sub_dataset_generate_multi_sample(circuits_info, output_filename, environment, repetitions):
    data = [
        ['circuit_code', 'width', 'depth', 'gate_number', 'multi_gate_number', 'fidelity', 'e_noise', 'noise_x',
         'noise_y', 'noise_z']]
    ...
    for circuit_info, all_count in zip(circuits_info, range(len(circuits_info))):
        ...
        fidelity = get_bas(circuit_code, width, environment, repetitions)
...
if __name__ == '__main__':
    ...
    dataset_generate_by_certain('OtherXlsx/zkd_circuit_each_all_struct_200.xlsx', 'GNN_training_data_6000_13_gate_simulate_e3', [0.020, 0.060, 0.020], 1000)
